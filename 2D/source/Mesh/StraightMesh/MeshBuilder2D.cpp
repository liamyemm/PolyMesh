...
namespace PolyMesh2D
{
    ...
    {
        double shortest_distance(VectorRd p, VectorRd A, VectorRd B) // computes min distance from a point p to a line segment AB
        {
        ...
        std::vector<Triangle> simplexify(std::vector<VectorRd> vertices)
        {
            ...
            while (vertices.size() > 3)
            {
                ...
                simplices.push_back(Triangle({v1, v2, v3}));
            ...
            simplices.push_back(Triangle({vertices[0], vertices[1], vertices[2]})); // final simplex is the remaining vertices
        ...
        std::unique_ptr<Mesh> MeshBuilder::build_the_mesh()
        {
            ...
            if (vertices.size() > 0 && cells.size() > 0)
            {
                ...
                for (auto &c : cells)
                {
                    ...
                    std::vector<Triangle> simplices = simplexify(vertex_coords);
                    ...
                    for (std::size_t i = 0; i < vertex_ids.size(); ++i)
                    {
                        ...
                        if (!edge_exists)
                        {
                            ...
                            Edge *edge = new Edge(mesh->n_edges(), {vertex_coords[i], vertex_coords[i_next]});
        ...
        void MeshBuilder::build_boundary(Mesh *mesh) ///< identifies boundary cells and vertices, and compute lists of boundary cells, edges and vertices
        {
            for (auto &edge : mesh->get_edges())
            {
                if (edge->n_cells() == 1) // If edge has only one cell, it is a boundary edge
                {
                    ...
                    edge->get_cells()[0]->set_boundary(true); // set cell boundary to true

                    for (auto &vert : edge->get_vertices())
                    {
                        vert->set_boundary(true); // set vertex boundary to true
                    }

                    mesh->add_b_edge(edge); // add edge to list of boundary edges
                }
                else
                {
                    // internal edge
                    mesh->add_i_edge(edge); // add edge to list of internal edges
                }
            }
