...
namespace Mesh3D
{
    ...
    public:
        ...
        inline std::vector<Face*> get_faces() const { return _faces; }         ///< lists the faces in the mesh.
        ...
        inline std::vector<Face*> get_b_faces() const { return _b_faces; }         ///< lists the boundary faces in the mesh.
        ...
        inline std::vector<Face*> get_i_faces() const { return _i_faces; }         ///< lists the internal faces in the mesh.
        ...
        void add_edge(Edge*edge) ///<  adds a edge to the mesh
        {
            ...
            // if (dimension == 2)
            // {
                assert(std::find(_faces.begin(), _faces.end(), reinterpret_cast<Face*>(edge)) == _faces.end());
                _faces.push_back(reinterpret_cast<Face*>(edge));
            // }            
        }
        void add_face(Face*face) ///<  adds a face to the mesh
        {
            ...
            // if (dimension == 2)
            // {
                assert(std::find(_edges.begin(), _edges.end(), reinterpret_cast<Edge*>(face)) == _edges.end());
                _edges.push_back(reinterpret_cast<Edge*>(face));
            // }
        }
        ...
        void add_b_edge(Edge*edge) ///<  adds a boundary edge to the mesh
        {
            ...
            // if (dimension == 2)
            // {
                assert(std::find(_b_faces.begin(), _b_faces.end(), reinterpret_cast<Face*>(edge)) == _b_faces.end());
                _b_faces.push_back(reinterpret_cast<Face*>(edge));
            // }
        }
        void add_b_face(Face*face) ///<  adds a boundary face to the mesh
        {
            ...
            // if (dimension == 2)
            // {
                assert(std::find(_b_edges.begin(), _b_edges.end(), reinterpret_cast<Edge*>(face)) == _b_edges.end());
                _b_edges.push_back(reinterpret_cast<Edge*>(face));
            // }
        }
        ...
        void add_i_edge(Edge*edge) ///<  adds an internal edge to the mesh
        {
            ...
            // if (dimension == 2)
            // {
                assert(std::find(_i_faces.begin(), _i_faces.end(), reinterpret_cast<Face*>(edge)) == _i_faces.end());
                _i_faces.push_back(reinterpret_cast<Face*>(edge));
            // }
        }
        void add_i_face(Face*face) ///<  adds an internal face to the mesh
        {
            ...
            // if (dimension == 2)
            // {
                assert(std::find(_i_edges.begin(), _i_edges.end(), reinterpret_cast<Edge*>(face)) == _i_edges.end());
                _i_edges.push_back(reinterpret_cast<Edge*>(face));
            // }
        }
        ...
        inline Face* face(std::size_t index) const { assert(index < _faces.size()); return _faces[index]; }        ///<  get a constant pointer to a face using its global index
        ...
        inline Face* b_face(std::size_t index) const { assert(index < _b_faces.size()); return _b_faces[index]; }        ///<  get a constant pointer to boundary a face using an index
        ...
        inline Face *i_face(std::size_t index) const { assert(index < _i_faces.size()); return _i_faces[index]; }        ///<  get a constant pointer to an internal face using an index
