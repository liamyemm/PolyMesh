...
namespace Mesh3D
{
    ...
    */
    ...
    public:
        ...
        inline std::vector<Polytope<DIMENSION - 1>*> get_faces() const { return _faces; } ///< Return the faces of the Polytope
        ...
        inline size_t n_faces() const { return _faces.size(); }       ///< Return the number of faces of the Polytope
    ...
    void Polytope<object_dim>::construct_face_normals() // not very efficient - probably room for improvement
    {
        ...
        {
            VectorRd normal = _faces[iF]->normal();
            Simplex<object_dim - 1> face_simplex = _faces[iF]->get_simplices()[0];
    ...
    {
        assert(object_dim == DIMENSION); // must be a cell
    ...
    {
        assert(object_dim == 2); // must be a 2D object (face)
        assert(edge_index < _edges.size());

        VectorRd edge_normal = _normal.cross(_edges[edge_index]->tangent()); // unit vector as normal and tangent are orthogonal unit vectors
        return edge_normal;
    }
    ...
    int Polytope<object_dim>::edge_orientation(const size_t edge_index) const
    {
        assert(object_dim == 2);
        assert(edge_index < _edges.size());
        return Math::sgn((_edges[edge_index]->center_mass() - this->center_mass()).dot(this->edge_normal(edge_index))); // assuming face is star-shaped wrt cell-center !!
    }
