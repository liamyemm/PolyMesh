...
void VtuWriter::write_cell_scalar_property(FILE *pFile,
                                           Eigen::VectorXd data,
                                           std::string name)
{
...
bool VtuWriter::write_cells_to_vtu(std::string filename, Eigen::VectorXd sol_vertex)
{
     FILE *pFile = fopen(filename.c_str(), "w");
     write_header(pFile);
     write_vertices(pFile);
     write_cells(pFile);
     write_cell_scalar_property(pFile, sol_vertex, "solution");
     write_footer(pFile);
     fclose(pFile);
     return true;
}
