...
using namespace PolyMesh2D::HHOPOISSON;
...
std::vector<double> Model::compute_errors(const Eigen::VectorXd &approx_Uvec, const Eigen::VectorXd &interp_Uvec, const ScalarFunction2D &sol)
{
    ...
    for (size_t iT = 0; iT < mesh_ptr->n_cells(); ++iT)
    {
        ...
        // L2_diff += diff_vec.head(m_hho.local_cell_dofs(iT)).squaredNorm();
        // L2_exact += ITk.head(m_hho.local_cell_dofs(iT)).squaredNorm();

        // L2_diff += mesh_ptr->cell(iT)->diam() * diff_vec.tail(m_hho.local_boundary_dofs(iT)).squaredNorm();
        // L2_exact += mesh_ptr->cell(iT)->diam() * ITk.tail(m_hho.local_boundary_dofs(iT)).squaredNorm();

        // QuadratureRule<Eigen::Vector2d> quadT = m_hho.quadT(iT);
        // size_t n_local_highorder_dofs = m_hho.local_highorder_dofs(iT);
        ...
        // {
        //     for (size_t j = i; j < n_local_highorder_dofs; ++j)
        //     {
        //         for (size_t iqn = 0; iqn < quadT.size(); ++iqn)
        //         {
        //             ST(i, j) += quadT[iqn].w * m_hho.highorder_basis(iT)->derivative(i, quadT[iqn].x) * (m_hho.highorder_basis(iT)->derivative(j, quadT[iqn].x)).transpose();
        //         }
        //         ST(j, i) = ST(i, j);
        //     }
        // }
        ...
        // H1_diff += (PT[iT] * diff_vec).transpose() * ST * (PT[iT] * diff_vec);
    }

    // for (size_t iE = 0; iE < mesh_ptr->n_edges(); ++iE)
    // {
    //     Edge *edge = mesh_ptr->edge(iE);
    //     if (edge->is_boundary())
    //     {
    //         continue;
    //     }
    //     size_t iT1 = edge->cell(0)->global_index();
    //     size_t iT2 = edge->cell(1)->global_index();

    //     Eigen::VectorXd PT1 = PT[iT1] * m_hho.restr(approx_Uvec, iT1);
    //     Eigen::VectorXd PT2 = PT[iT2] * m_hho.restr(approx_Uvec, iT2);

    //     QuadratureRule<double> quadE = m_hho.quadE(iE);
    //     size_t n_local_edge_dofs = m_hho.local_edge_dofs(iE);
    //     size_t n_local_highorder_dofs_1 = m_hho.local_highorder_dofs(iT1);
    //     size_t n_local_highorder_dofs_2 = m_hho.local_highorder_dofs(iT2);

    //     Eigen::MatrixXd MFF = Eigen::MatrixXd::Zero(n_local_edge_dofs, n_local_edge_dofs);
    //     Eigen::MatrixXd MFT1 = Eigen::MatrixXd::Zero(n_local_edge_dofs, n_local_highorder_dofs_1);
    //     Eigen::MatrixXd MFT2 = Eigen::MatrixXd::Zero(n_local_edge_dofs, n_local_highorder_dofs_2);

    //     for (size_t i = 0; i < n_local_edge_dofs; ++i)
    //     {
    //         for (size_t iqn = 0; iqn < quadE.size(); ++iqn)
    //         {
    //             double q_val = quadE[iqn].x;
    //             double q_weight = quadE[iqn].w;
    //             for (size_t j = 0; j < n_local_edge_dofs; ++j)
    //             {
    //                 MFF(i, j) += q_weight * m_hho.edge_basis(iE)->value(i, q_val) * m_hho.edge_basis(iE)->value(j, q_val);
    //             }
    //             for (size_t j = 0; j < n_local_highorder_dofs_1; ++j)
    //             {
    //                 MFT1(i, j) += q_weight * m_hho.edge_basis(iE)->value(i, q_val) * m_hho.highorder_basis(iT1)->value(j, edge->parameterisation().value(q_val));
    //             }
    //             for (size_t j = 0; j < n_local_highorder_dofs_2; ++j)
    //             {
    //                 MFT2(i, j) += q_weight* m_hho.edge_basis(iE)->value(i, q_val) * m_hho.highorder_basis(iT2)->value(j, edge->parameterisation().value(q_val));
    //             }
    //         }
    //     }

    //     Eigen::VectorXd PT1_minus_PT2_on_F = MFF.inverse() * (MFT1 * PT1 - MFT2 * PT2);
    //     H1_diff += ( (1.0 / edge->cell(0)->diam()) + (1.0 / edge->cell(1)->diam()) ) * PT1_minus_PT2_on_F.transpose() * MFF * PT1_minus_PT2_on_F;
    // }
...
void Model::local_poisson_operator(const size_t iT, Eigen::MatrixXd &AT, Eigen::MatrixXd &PT)
{
    ...
    PT = (ST + scalT * LTtLT).ldlt().solve(BP);
    Eigen::MatrixXd ATCONS = PT.transpose() * ST * PT;

    Eigen::MatrixXd DT = MTT.topLeftCorner(n_local_cell_dofs, n_local_cell_dofs).inverse() * MTT * PT;
    ...
    Eigen::MatrixXd STAB_OPERATOR = M_BDRY_BDRY_INV_M_BDRY_T.topLeftCorner(n_local_bdry_dofs, n_local_highorder_dofs) * PT;
    ...
    Eigen::MatrixXd ATSTAB = DT.transpose() * ST.topLeftCorner(n_local_cell_dofs, n_local_cell_dofs) * DT + (1.0 / cell->diam()) * STAB_OPERATOR.transpose() * M_BDRY_BDRY * STAB_OPERATOR;
